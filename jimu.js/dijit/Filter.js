    ...
    */
    build: function(options){
      ...
      if(this.isBuilding()){
        ...
        this.layerTimeZoneOffset = this._filterUtils.getLayerTimezoneOffset(options.layerDefinition)
        if(options.partsObj){
    ...
    // Handle `_useStandardizedQueries` and `_layerTimeZoneOffset` prop for Set, Single experience
    _handleUseStandardizedQueriesAndTimeZone: function(part) {
      if (part) {
        if (part.parts) {
          array.forEach(part.parts, lang.hitch(this, function(singlePart){
            if (singlePart.fieldObj.shortType === 'date') {
              if (this.useStandardizedQueries) {
          ...
          part._layerTimeZoneOffset = this.layerTimeZoneOffset
        }
      }
      return part
    },
    ...
    toJson:function(){
      ...
      array.forEach(filters, lang.hitch(this, function(filter){
        ...
        part = this._handleUseStandardizedQueriesAndTimeZone(part)
